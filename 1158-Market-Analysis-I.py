...
def market_analysis(users: pd.DataFrame, orders: pd.DataFrame, items: pd.DataFrame) -> pd.DataFrame:
    merged1 = pd.merge(users, orders, left_on="user_id", right_on="buyer_id", how="left")
    ...
    filtered = filtered.groupby('user_id').size().reindex(all_users, fill_value=0).reset_index(drop=True)
